...
public class TagRepositoryImpl implements TagRepository {
	...
	public List<Tag> findAll(List<String> languages, String sort, String order) {
		...
		List<TagMongoEntity> tags = tagMongoRepository.findAll(languages, Sort.by(Sort.Direction.fromString(order), sort));
	...
	public void setImage(String source, String image) {
		TagMongoEntity entity = tagMongoRepository.findById(source).get();
	...
	public void merge(String source, String target) {
		TagMongoEntity entitySource = tagMongoRepository.findById(source).get();
		TagMongoEntity entityTarget = tagMongoRepository.findById(target).get();
		...
		for (String lang : entitySource.getNumBooks().getLanguages().keySet()) {
	...
	public void rename(String source, String target) {
		...
		for (BookMongoEntity book : books) {
			book.getTags().replaceAll(b -> b.equals(entity.getName()) ? target : b);
		}
		bookMongoRepository.saveAll(books);
		...
		tags.forEach(tag -> {
			...
			if (_entity.isEmpty()) {
				...
				languages.forEach(lang -> {
					if (entity.getNumBooks().getLanguages().get(lang) != null) {
						entity.getNumBooks().getLanguages().put(lang, entity.getNumBooks().getLanguages().get(lang) + 1);
					}
					else {
						entity.getNumBooks().getLanguages().put(lang, 1);
					}
				});
				entity.getNumBooks().setTotal(entity.getNumBooks().getTotal() + 1);
				tagMongoRepository.save(entity);
			}
		});
